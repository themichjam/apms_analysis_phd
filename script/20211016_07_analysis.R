...
### new workflow ###
## Follow up
tbl wgt unw sdo to do, 

Factors (4) – you still need to control the ordering of levels within the factor so they fall in more logical order (‘levels’ subcommand) – see figure on income quintiles, e.g.

XX Sorting unwanted NAs (3) – not sure this is working, see e.g. income quintiles where ‘Item not applicable’ still appears unless filtered out. Do some checks at this stage to prove to yourself it is doing what you expected. XX

Weights/survey design (s.5) –Reading up a bit, I found this page v. clear and helpful: https://stats.idre.ucla.edu/r/seminars/survey-data-analysis-with-r/. The main thing I took from it was the need to produce tables, figs etc. using other commands from the ‘survey’ package (svytable, svyhist, etc.). The svydesign command which you use creates an object which describes the complex design, but you then need to use the svy*** commands to apply that design object to the data correctly. You don’t seem to be doing it this way but maybe you’ve got a different way of achieving the same thing? 
  
  At a few points, I see you use ‘ggsurvey’ for figures and that does take the svydesign object BUT (v unhelpfully) the small print for the documentation says that this only works with weights, not with complex design: (https://www.rdocumentation.org/packages/questionr/versions/0.7.4/topics/ggsurvey – under ‘Details’). 

There are ‘survey’ commands for modelling as well which you’d also need to use (s.8). 
# clear enviroment 
rm(list = ls())
...
here()
# Read in 2007
source_pms07 <- read_sav("data/raw/apms07arch.sav", 
                         user_na = TRUE) %>%
  as_factor() 
...
subtest <- subset_pms07 %>%
  filter_at(vars(starts_with("acid")), any_vars(! is.na(.)))

table(subset_pms07$acidever)
colSums(is.na(subset_pms07)) # count NAs in df
na.fail(subset_pms07) # returns error if NAs present
...
# Preparing Weights for Use in Dataset
...
#ggsave(filename=here("figs/2007", "uniage.png"))
#Difference with weighting.
# weighted age
