...
Sorting unwanted NAs (3) – not sure this is working, see e.g. income quintiles where ‘Item not applicable’ still appears unless filtered out. Do some checks at this stage to prove to yourself it is doing what you expected.
...
# Read in 2007
source_pms07 <- haven::read_sav("07_analysis/apms07arch.sav", 
                                user_na = TRUE) %>%
  as_factor() 
```
...
Using the ```svydesign``` object combines a data frame (subset_pms07) and all the survey design information needed to analyse it. These objects are used by the survey modelling and summary functions, as well as visulisations.

In this set ```cluster``` is the stratification variable, ```area``` the Primary sampling unit variable. PSU is sampled within each strata. ```wt_ints1```is the survey weight for all analyses except specialised ones for conditions. 
```{r weight prep}
...
Overview of whole subset of interest. 
...
Unweighted vs Weighted Plots
```{r weight and unw plots}
...
1. Can't seem to filter when plotting with a weighted survey object. 
